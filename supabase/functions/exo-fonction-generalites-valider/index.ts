// Valide la reponse d'un eleve pour le template fonction_generalites_v1 (QCM : reponse = index 0-3).
...
async function decryptToken(token: string): Promise<{ idx: number; correction: string; template: string; ts: number }> {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { token, reponse, exercice_id, chapitre, niveau, enonce, choix } = await req.json();
    ...
    const correcte = Number.isInteger(donnee) && donnee === payload.idx;
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "fonction_generalites_v1",
        chapitre: chapitre ?? "Fonctions — généralités",
        niveau: niveau ?? "3eme",
        score, points_obtenus: correcte ? 1 : 0, points_total: 1,
        completed_at: new Date().toISOString(),
        enonce: enonce ?? null,
        reponse_donnee: Array.isArray(choix) && choix[donnee] !== undefined ? choix[donnee] : String(reponse),
        reponse_attendue: Array.isArray(choix) && choix[payload.idx] !== undefined ? choix[payload.idx] : String(payload.idx),
      }),
    });
    ...
    return new Response(JSON.stringify({ correcte, score, bonIdx: payload.idx, correction: payload.correction }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
