// Valide la reponse d'un eleve pour le template suite_premiers_termes_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "suite_premiers_termes_v1",
        chapitre: chapitre ?? "Suites — définition et premiers termes",
        niveau: niveau ?? "1ere",
