// Valide la reponse d'un eleve pour le template logarithme_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const correcte = donnee !== null && Math.abs(donnee - payload.x) < 0.01;
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "logarithme_v1",
        chapitre: chapitre ?? "Logarithme népérien",
