// Valide la reponse d'un eleve pour le template appartenance_plan_v1.
  ...
  try {
    ...
    if (eleveId) {
      ...
      const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
        ...
        body: JSON.stringify({
          eleve_id: eleveId,
          exercice_id: exercice_id ?? "appartenance_plan_v1",
          chapitre: chapitre ?? "Droites et plans de l'espace",
