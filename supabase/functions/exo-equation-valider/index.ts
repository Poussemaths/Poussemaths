// Valide la reponse d'un eleve pour le template equation_1er_degre_v1.
// Dechiffre le token pour retrouver la bonne reponse (jamais transmise en clair),
// calcule le score cote serveur, et ecrit progression via la cle service_role
// (le client n'a plus jamais le droit d'ecrire directement un score).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(
      `${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`,
      {
        method: "POST",
        headers: {
          apikey: serviceKey!,
        ...
        body: JSON.stringify({
          eleve_id: eleveId,
          exercice_id: exercice_id ?? "equation_1er_degre_v1",
          chapitre: chapitre ?? "equations",
          niveau: niveau ?? "3eme",
          score,
