// Valide la reponse d'un eleve pour le template equadiff_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Tolerance liee a la precision d'affichage reelle (voir roundSmart cote
    // generateur : 4/3/2 decimales selon l'echelle), PAS un pourcentage de la
    // valeur -- une tolerance relative (ex: 1%) accepterait a tort un ecart
    // de plusieurs unites sur une grande valeur (bug trouve et corrige avant
    // deploiement : ecart de 20 sur une reponse de 2017 accepte a tort).
    const absX = Math.abs(payload.x);
    const tolerance = absX < 1 ? 0.001 : absX < 10 ? 0.01 : 0.1;
    const correcte = donnee !== null && Math.abs(donnee - payload.x) < tolerance;
    ...
    if (eleveId) {
      ...
      const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
        ...
        body: JSON.stringify({
          eleve_id: eleveId,
          exercice_id: exercice_id ?? "equadiff_v1",
          chapitre: chapitre ?? "Équations différentielles",
