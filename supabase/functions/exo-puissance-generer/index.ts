// Template : calcul de puissance a^n, saisie libre (nombre entier).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const enonce = `Calcule : $${a}^${n}$`;
    const token = await encryptPayload({ x: reponse, template: "puissance_v1", ts: Date.now() });
