// Template : distance entre deux nombres sur la droite graduee (valeur absolue), saisie libre.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const token = await encryptPayload({ x: reponse, template: "valeur_absolue_v1", ts: Date.now() });
