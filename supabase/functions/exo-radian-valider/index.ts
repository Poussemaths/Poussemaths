// Valide la reponse d'un eleve pour le template radian_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Tolerance plus large (0,005) : la valeur attendue est deja arrondie au
    // centieme au moment de la generation, on accepte tout ce qui arrondit
    // pareil (evite un rejet a tort sur un dernier chiffre de representation
    // flottante).
    const correcte = donnee !== null && Math.abs(donnee - payload.x) < 0.005;
    ...
    if (eleveId) {
      ...
      const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
        ...
        body: JSON.stringify({
          eleve_id: eleveId,
          exercice_id: exercice_id ?? "radian_v1",
          chapitre: chapitre ?? "Trigonométrie — cercle trigonométrique",
          niveau: niveau ?? "1ere",
