// Valide la reponse d'un eleve pour le template fonction_reference_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "fonction_reference_v1",
        chapitre: chapitre ?? "Fonctions de référence",
        niveau: niveau ?? "2nde",
