// Template : calcul de moyenne d'une serie statistique, saisie libre (nombre).
// Les valeurs sont ajustees pour garantir une moyenne entiere exacte.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const token = await encryptPayload({ x: moyenne, template: "moyenne_v1", ts: Date.now() });
