// Valide la reponse d'un eleve pour le template fraction_addition_v1.
// Tolerance elargie car le denominateur peut donner un decimal non-terminal
// (l'eleve est invite a repondre en fraction, forme exacte, mais un decimal
// raisonnablement arrondi doit aussi passer).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const correcte = donnee !== null && Math.abs(donnee - payload.x) < 0.006;
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "fraction_addition_v1",
        chapitre: chapitre ?? "Fractions",
        niveau: niveau ?? "6eme",
