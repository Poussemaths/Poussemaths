// Valide la reponse d'un eleve pour le template trigonometrie_v1.
// Tolerance elargie : certains triplets donnent des decimales non-terminales
// (ex: 5/13), la forme fraction exacte reste toujours acceptee sans marge d'erreur.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "trigonometrie_v1",
        chapitre: chapitre ?? "Trigonométrie",
        niveau: niveau ?? "3eme",
