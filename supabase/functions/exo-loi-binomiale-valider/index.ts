// Valide la reponse d'un eleve pour le template loi_binomiale_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Tolerance plus large (valeur arrondie a 4 decimales demandee a l'eleve,
    // qui peut legitimement arrondir differemment son calcul intermediaire).
    ...
    if (eleveId) {
      ...
      const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
        ...
        body: JSON.stringify({
          eleve_id: eleveId,
          exercice_id: exercice_id ?? "loi_binomiale_v1",
          chapitre: chapitre ?? "Loi binomiale",
