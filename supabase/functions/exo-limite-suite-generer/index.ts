// Template : limite d'une suite rationnelle u_n = (a n + b) / (c n + d) quand n -> +infini.
// La limite vaut a/c (le degre du numerateur et du denominateur est le meme).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const token = await encryptPayload({ x: reponse, template: "limite_suite_v1", ts: Date.now() });
