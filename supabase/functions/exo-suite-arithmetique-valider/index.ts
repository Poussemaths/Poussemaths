// Valide la reponse d'un eleve pour le template suite_arithmetique_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "suite_arithmetique_v1",
        chapitre: chapitre ?? "Suites arithmétiques",
        niveau: niveau ?? "1ere",
