// Template : racine carree d'un carre parfait, saisie libre (nombre entier).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const enonce = `Calcule : $\\sqrt{${n}}$`;
    const token = await encryptPayload({ x: k, template: "racine_carree_v1", ts: Date.now() });
