// Template : addition/soustraction de nombres relatifs, saisie libre (nombre entier).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const token = await encryptPayload({ x: reponse, template: "relatifs_v1", ts: Date.now() });
