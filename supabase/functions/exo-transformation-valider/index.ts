// Valide la reponse d'un eleve pour le template transformation_v1.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const writeResp = await fetch(`${supabaseUrl}/rest/v1/progression?on_conflict=eleve_id,exercice_id`, {
      ...
      body: JSON.stringify({
        eleve_id: eleveId,
        exercice_id: exercice_id ?? "transformation_v1",
        chapitre: chapitre ?? "Transformations géométriques",
        niveau: niveau ?? "3eme",
